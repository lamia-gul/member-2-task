  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.countText}>{habits.length} Plants</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, item.streak >= item.duration && styles.completedCard]}>
            ...
            <TouchableOpacity 
              style={[styles.check, item.streak >= item.duration && { backgroundColor: '#FFD700' }]} 
            ...
            <TouchableOpacity onPress={() => dispatch(deleteHabit(item.id))} style={styles.delete}>
              <Text style={{color: 'white', fontSize: 10}}>✕</Text>
            </TouchableOpacity>
          </View>
        )}
        ListEmptyComponent={<Text style={styles.emptyText}>Your garden is quiet... plant something! 🌱</Text>}
      />
...
const styles = StyleSheet.create({
  ...
  addBtn: { backgroundColor: '#2E7D32', padding: 18, borderRadius: 20, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.2, elevation: 5 },
